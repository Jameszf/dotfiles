...
* Issues
 ...
 + Add extra code to make fresh starts of config easiser (e.g. making `.saves` and `org` folders if not detected in `.emacs.d`).
 + Look into devdocs package and other not-tangled packages.
...
** Devdocs
A package for viewing documentation in an Emacs buffer. It supports over 100+ technologies. Documentation must be installed first using =devdocs-install= before viewable with =devdocs-lookup=. 
#+begin_src emacs-lisp :tangle no
